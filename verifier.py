...
questions_par_tp = { \
    ...
    2 : # TP2
    {
        ...
        QuestionKey(8) :
            QuestionATester([ClassInfo('Dvector',False)],
                            [],
                            QuestionATester.getSingleTest(['const Dvector v1(3)',
                                                           'Dvector v2 = v1.view( true, 1, 1 )',
                                                           'bool owner = v1.isOwner()',
                                                           'bool isSamePtr = v1.getData() == v2.getData()']),
                            QuestionList([1]))
    },
    3: # TP3
    {
    ...
    4: # TP4
    {
        ...
        QuestionKey(5) :
            QuestionATester([ClassInfo('Point',True),ClassInfo('Triangle',True),ClassInfo('Maillage',True)],
                            ...
                            QuestionATester.getSingleTest( ['Point<double> p1( 0, 0 )',
                                                            'Point<double> p2( 5, 0 )',
                                                            'Maillage<double,std::vector> m1( 5, 5, p1 )',
                                                            'Maillage<double,std::vector> m2( 5, 5, p2 )',
                                                            'm1.fusionner( m2 )'] ),
                            QuestionList([1,2,3])),
        QuestionKey(6) :
            QuestionATester([ClassInfo('Point',True),ClassInfo('Triangle',True),ClassInfo('Maillage',True)],
...
def createMakefile( folder, project, files, includes ):
    ...
    makefile.write( "CXXFLAGS = -Wall -g" + cxxExtraFlags + "\n" )
    incs = ""
